...
public:
	...
	void ReqQryOrder()
	{
		...
		//qry.FilterFlag = DERIVATIVE_ORD_FILTER_L;	//查询当日未全部成交的排队单/已请求单
		//或者
		qry.FilterFlag = DERIVATIVE_ORD_FILTER_A;	//查询当日所有单
	...
	void ReqOrderInsert()
	{
		...
		*/
		...
		CFutureReqOrderInsertField req = {};
		string localID = BuildLocalID(1);

		safe_cpy(req.UserId, m_appConfig.userId.c_str());
		//safe_cpy(req.AccountNo, find->second.AccountNo); 
		safe_cpy(req.AccountNo, "Tiger Acct");
		safe_cpy(req.LocalNo, localID.c_str());
		safe_cpy(req.ExchangeCode, "ICE");
		safe_cpy(req.ContractCode, "BRN2312");
		safe_cpy(req.TIF, DERIVATIVE_GTC_TIF);
		safe_cpy(req.BidAskFlag, DERIVATIVE_ASK);
		safe_cpy(req.OrderQty, "1");
		safe_cpy(req.OrderPrice, "94");
		safe_cpy(req.Tag50, "ywh");
		

		//safe_cpy(req.TreatyCode, "A-UC2009");
		//safe_cpy(req.OrderPrice, "7.1064");

		safe_cpy(req.OrderType, DERIVATIVE_LIMIT_ORDER);
	
		// 如果跨期合约类别，需要送ContractCategory
		//req.ContractCategory = DERIVATIVE_CONTRACT_CATEGORY_SPREAD;
	...
	void ReqOrderModify()
	{
		...
		*/
		...
		CFutureReqOrderModifyField req = {};
		...
		safe_cpy(req.ModifyQty, "2");
		safe_cpy(req.ModifyPrice, "93");
	...
	void ReqOrderCancel()
	{
		...
		*/
		...
		CFutureReqOrderCancelField req = {};
		string localID = BuildLocalID(1);
